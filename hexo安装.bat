...
echo.
echo     重新渲染
start cmd /k "cd %userprofile%\Desktop\blog && hexo clean && hexo clean&& hexo g && hexo s"
echo.
echo   清楚缓存，重新渲染，开启服务
echo.
echo     5秒后自动返回主菜单
...
echo.
echo     上传服务器比如github
start cmd /k "cd %userprofile%\Desktop\blog && hexo d"
echo.
echo   上传文件到服务器
echo.
echo     5秒后自动返回主菜单
...
echo.
echo     安装所需插件
start cmd /k "cd %userprofile%\Desktop\blog && npm install --save hexo-tag-aplayer && npm install hexo-abbrlink --save && npm install hexo-butterfly-envelope --save && npm install hexo-lazyload-image --save "
echo.
echo   安装所需插件
echo.
echo     5秒后自动返回主菜单
